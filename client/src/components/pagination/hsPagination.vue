<template lang="pug">
  div(class="pagination")
    div(class="pagination__information")
      div(class="pagination__information__text") {{configPagination.information.label}}
      div(class="pagination__information__text") {{configPagination.information.total}}

    div(class="pagination__buttons")
      div(class="pagination__buttons__start")
        button(
          type="button"
          value="1"
          class="pagination__buttons__start__origin-button"
          @click="sendPage($event)") {{lang.hsPagination.buttonStart}}
        button(
          type="button"
          class="pagination__buttons__start__before-button"
          @click="decrease()")
            icon(name="caret-left")

      div(class="pagination__buttons__middle")
        button(
          :ref="configPagination.id"
          v-for="page in makingButtons"
          type="button"
          :value="page"
          class="pagination__buttons__middle__cental-button"
          @click="sendPage($event)") {{page}}

      div(class="pagination__buttons__end")
        button(
          type="button"
          class="pagination__buttons__end__after-button"
          @click="increase()")
            icon(name="caret-right")
        button(
          type="button"
          :value="configPagination.buttons.pages"
          class="pagination__buttons__end__final-button"
...
export default {
  ...
  watch: {
    chainButtons (data) {
      // ACTIVAR COLOR AL NUMERO ACTIVO
      console.log(this.$refs[this.configPagination.id])
    }
  },
  ...
  computed: {
    makingButtons () {
      if (this.configPagination.buttons.buttons > this.configPagination.buttons.pages) {
        console.error('No hay suficientes paginas')
