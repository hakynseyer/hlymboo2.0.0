...
const valServer = (lang) => {
  ...
  const langData = boards(lang, 'register', 'validator')
  ...
  return {
    async uniqueAlias (alias) {
      try {
        ...
        if (userID !== null) {
          if (userID.id) errors.alias.push(langData.registerValidator.uniqueAlias.res)
        }
      } catch (error) {
        console.error(error)
        errors.alias.push(langData.registerValidator.uniqueAlias.resError)
      }
    },
    async uniqueEmail (email) {
      try {
        ...
        if (userID !== null) {
          if (userID.id) errors.email.push(langData.registerValidator.uniqueEmail.res)
        }
      } catch (error) {
        console.error(error)
        errors.email.push(langData.registerValidator.uniqueEmail.resError)
      }
    }
  }
}
